  ...
  def return_matches(hashes):
    ...
    for split_values in grouper(values, 1000):
      ...
      for hash, sid, offset in x:
        ...
        offest_int =int.from_bytes(offset,byteorder='little',signed = False) 
        print("Mapper[hash]",mapper[hash])
        print("Mapper[hash]type",type(mapper[hash]))
        print("offest_int",offest_int)
        print("offest_int type",type(offest_int))
        yield (sid, offest_int - mapper[hash])
